...
describe("Task 3: arrayManipulation", () => {
  it("should correctly manipulate the array", () => {
    ...
    const result = arrayManipulation(
      initialArray,
      "Calf",
      "Bear",
      "Elephant",
...
describe("Task 6: unique", () => {
  it("should return an array with only unique values", () => {
    ...
    expect(unique(colors)).toEqual(
      expect.arrayContaining(["red", "green", "blue", "yellow", "orange"]),
    );
  });
});
...
describe("Task 7: processBooks", () => {
  ...
  it("should add a genre property to all books", () => {
    expect(results.booksWithGenre.every((b) => b.genre === "classic")).toBe(
      true,
    );
  });
});
...
describe("Task 8: managePhoneBook", () => {
  ...
  const combined = managePhoneBook(
    phoneBookABC,
    phoneBookDEF,
    newCarolineNumber,
  );
